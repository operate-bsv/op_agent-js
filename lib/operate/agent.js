...
class Agent {
  ...
  async loadTape(txid, opts = {}) {
    ...
    return await this._prepTape(tx, config)
  }
  ...
  async loadTapesBy(query, opts = {}) {
    ...
    return await this._prepTape(txns, config)
  }
   ...
   * @private
   */
  async _prepTape(tx, index = 0, opts = {}) {
    const config = {
      ...this.config,
      ...opts
    }
    ...
    if (Array.isArray(tx)) {
      const tapes = await Promise.all(
        tx.map(async tx => this._prepTape(tx, opts))
      )
