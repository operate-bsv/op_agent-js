...
class Agent {
  ...
  async loadTape(txid, opts = {}) {
    ...
    let [cache, cacheConfig] = Agent.adapterWithOpts(config.cache);
    const adapter = Agent.adapterWithOpts(config.tape_adapter);
  ...
  async prepTape(tx, opts = {}) {
    ...
    let [cache, cacheConfig] = Agent.adapterWithOpts(config.cache);
    const adapter = Agent.adapterWithOpts(config.op_adapter);
   ...
   *
   * @private
   * @param {Array|Adapter} adapter
   */
  static adapterWithOpts(adapter) {
