...
class Agent extends Extension {
  static extend(vm) {
    ...
    vm.setFunction("agent.load_tapes_by", (...args) =>
      this.loadTapesBy(vm, ...args)
    );
    vm.setFunction("agent.local_tape", (...args) =>
      this.localTape(vm, ...args)
    );
    vm.setFunction("agent.run_tape", (...args) => this.runTape(vm, ...args));
  }
